...
export const useExperiencesAdmin = () => {
  ...
  const handleUpdateAchievement = (experienceId: string, achievementIndex: number, value: string) => {
    setExperiences(prev =>
      prev.map(exp =>
        exp.id === experienceId
          ? {
    ...
    setExperiences(prev =>
      prev.map(exp =>
        exp.id === experienceId
          ? {
              ...exp,
              technologies: (exp.technologies || []).filter((_, idx) => idx !== techIndex),
            }
          : exp
      )
    );
    ...
    try {
      ...
      for (const experience of experiences) {
        ...
        if (!experience.achievements || experience.achievements.length === 0) {
          throw new Error(`Experience "${experience.id}" must have at least one achievement`);
        }
