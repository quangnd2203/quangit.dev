...
export const DashboardInsights = () => {
  ...
  const contactTrends = useMemo(() => {
    ...
    const responseRate = stats.contactMessages.total > 0
      ? ((stats.contactMessages.total - stats.contactMessages.unread) / stats.contactMessages.total * 100).toFixed(1)
      : '0';
  ...
  const portfolioOverview = useMemo(() => {
    const featuredPercentage = stats.projects.total > 0
        ...
        >
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-lg font-bold text-gray-900">{contactTrends.recent7Days}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-xs text-gray-600">Previous 7 days</span>
              <span className="text-sm font-medium text-gray-700">{contactTrends.previous7Days}</span>
            </div>
            <div className="flex items-center justify-between pt-2 border-t border-blue-200">
              <span className="text-xs text-gray-600">Response Rate</span>
              <span className="text-sm font-semibold text-blue-600">{contactTrends.responseRate}%</span>
            </div>
            {contactTrends.trend !== 0 && (
              <div className="flex items-center gap-1 pt-1">
                <span className={`text-xs font-medium ${contactTrends.trend > 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {contactTrends.trend > 0 ? '↑' : '↓'} {Math.abs(contactTrends.trend)}%
                </span>
        ...
        >
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-lg font-bold text-gray-900">{stats.projects.featured}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-xs text-gray-600">Featured %</span>
              <span className="text-sm font-semibold text-purple-600">{portfolioOverview.featuredPercentage}%</span>
            </div>
            ...
            {portfolioOverview.topTechnologies.length > 0 && (
              <div className="pt-2 border-t border-purple-200">
                ...
                <div className="space-y-1">
                  {portfolioOverview.topTechnologies.slice(0, 3).map(([tech, count]) => (
                    <div key={tech} className="flex items-center justify-between">
                      <span className="text-xs text-gray-700 truncate">{tech}</span>
                      <span className="text-xs font-medium text-gray-900">{count}</span>
                    </div>
                  ))}
                </div>
        ...
        >
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-lg font-bold text-gray-900">{stats.skills.total}</span>
            </div>
            <div className="pt-2 border-t border-orange-200 space-y-1.5">
              <div className="flex items-center justify-between">
                <span className="text-xs text-gray-700">Expert</span>
                <span className="text-xs font-semibold text-blue-600">{skillsDistribution.expert}%</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-gray-700">Advanced</span>
                <span className="text-xs font-semibold text-purple-600">{skillsDistribution.advanced}%</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-gray-700">Intermediate</span>
                <span className="text-xs font-semibold text-green-600">{skillsDistribution.intermediate}%</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-gray-700">Beginner</span>
                <span className="text-xs font-semibold text-gray-600">{skillsDistribution.beginner}%</span>
              </div>
            </div>
          </div>
        </motion.div>
