 ...
 */
export const createContactMessage = async (message: Omit<ContactMessage, 'id' | 'createdAt' | 'status'>): Promise<ContactMessage> => {
  try {
...
export const bulkUpdateContactMessageStatus = async (
  ...
  try {
    ...
    throw error instanceof Error ? error : new Error('Failed to bulk update contact message status');
  }
};
...
export const bulkToggleContactMessageImportant = async (
  ...
  try {
    ...
    throw error instanceof Error ? error : new Error('Failed to bulk toggle contact message important status');
  }
};
...
export const toggleContactMessageImportant = async (
  ...
  try {
    ...
    throw error instanceof Error ? error : new Error('Failed to toggle contact message important status');
  }
};
...
export const bulkDeleteContactMessages = async (ids: string[]): Promise<void> => {
  try {
    ...
    if (notFoundIds.length > 0 && notFoundIds.length === ids.length) {
      throw new Error(`No messages were found with the provided IDs: ${notFoundIds.join(', ')}`);
    }
